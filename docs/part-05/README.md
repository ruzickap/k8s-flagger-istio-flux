...
There are two additional services created
...
There is also new `VirtualService` created by Flagger:
...
You can also see 2 new `DestinationRules` added by Flagger:
...
Open the web browser and the the application: [http://podinfo.myexample.dev](http://podinfo.myexample.dev)
...
" C-m \; \
split-window -h -p 27 \; \
send-keys "while true; do curl -s http://podinfo.${MY_DOMAIN} | jq .message; sleep 1; done" C-m \; \
split-window -v -p 50 \; \
